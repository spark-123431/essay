...
class MagLoader:
    def __init__(self, material_path='', data_type='numpy', data_source='mat'):
        if material_path:
            ...
            self.b = self.h = self.temp = self.loss = np.array([])
        return
...
class MagDataset(Dataset):
    def __init__(self, file_path):
        ...
        self.x_data[:, :, 1] = mag_data.temp  # broadcast
        self.x_data[:, :, 2] = dB
        self.x_data[:, :, 3] = d2B
        self.x_data[:, :, 4] = mag_data.h
...
if __name__ == '__main__':
    # 设定路径
    raw_data_path = r"D:\essay\B5logfreq\materials"
    processed_data_dir = r"D:\essay\B5logfreq\Processed Training Data"
    newStep = 128
