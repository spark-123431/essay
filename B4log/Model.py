...
class CNNLSTMNet(nn.Module):
    def __init__(self, hidden_size, lstm_num_layers=1, input_size=1, output_size=1):
        ...
        # CNN 层：输入通道=2（B, h），输出通道=128
        self.cnn = nn.Sequential(
            nn.Conv1d(in_channels=2, out_channels=128, kernel_size=25, padding=12),
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        # 动态通道：B, h → [B, T, 2] → permute to [B, 2, T]
        dynamic = x[:, :, [0, 3]].permute(0, 2, 1)
...
def get_global_model():
    ...
    """
    return CNNLSTMNet(hidden_size=126, lstm_num_layers=3, input_size=2, output_size=1)
...
if __name__ == '__main__':
    ...
    # 构造最终 4 通道张量
    inputs = torch.stack([wave, freq, temp, wave_h], dim=2)  # shape [B, T, 4]
