...
for material in os.listdir(raw_data_path):
    ...
    try:
        temp = np.loadtxt(os.path.join(material_path, 'Temperature[C].csv'), delimiter=',').astype(np.float32)
        loss = np.loadtxt(os.path.join(material_path, 'Volumetric_losses[Wm-3].csv'), delimiter=',').astype(np.float32)
        ...
        # 保留有效数据
        mask = (temp > 0) & (loss > 0)
        all_temp.extend(temp[mask])
        all_loss.extend(loss[mask])
        print(f"已加载材料: {material}")
    except Exception as e:
        print(f"跳过材料 {material}，错误: {e}")
...
# 原始空间
axs[0].scatter(all_temp, all_loss, alpha=0.5, s=10, color='steelblue')
axs[0].set_title("Raw Temperature vs. Core Loss")
axs[0].set_xlabel("Temperature [°C]")
axs[0].set_ylabel("Volumetric Core Loss [W/m³]")
axs[0].grid(True)

# 对数变换空间
log_temp = np.log(all_temp + 1e-8)
log_loss = np.log(all_loss + 1e-8)
axs[1].scatter(log_temp, log_loss, alpha=0.5, s=10, color='darkorange')
axs[1].set_title("Log-Transformed Temperature vs. Log Loss")
axs[1].set_xlabel("log(Temperature)")
axs[1].set_ylabel("log(Core Loss)")
axs[1].grid(True)

plt.tight_layout()
plt.savefig(os.path.join(output_dir, "temp_vs_loss_log_compare.png"), dpi=300)
plt.show()
