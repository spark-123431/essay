...
if __name__ == "__main__":
    ...
    # 通用配置
    data_dir = "E:/project/B"
    weight_dir = os.path.join(data_dir, 'Trained Weights')
    base_material = "3C90"
    base_model_path = os.path.join(weight_dir, f"{base_material}.ckpt")  # 可选：填写预训练模型路径
    epochs = 900
    valid_batch_size = 64
    verbose = False
    ...
    print(f"\nFound {len(all_materials)} materials: {all_materials}")  # 作为 base model 的材料

    for material in all_materials:
        if material == base_material:
            print(f"[{material}] Skipped: Used as base model only.")
            continue

        base_model_path = os.path.join(weight_dir, f"{base_material}.ckpt")
        train_model(data_dir, material, base_model_path, device, epochs, valid_batch_size)