...
data_dir = r'D:\essay\B6logwithoutlstm'
...
for material in weights:
    ...
    pdf_path = os.path.join(validation_dir, f'{material}.pdf')
    png_path = os.path.join(validation_dir, f'{material}.png')
    DataProgress.magplot(material, relv_error, pdf_path)
    DataProgress.magplot(material, relv_error, png_path)
    ...
    # 同时保存 PDF 与 PNG
    pdf_line_path = os.path.join(validation_dir, f'{material}_loss_first_{n_plot}_samples.pdf')
    png_line_path = os.path.join(validation_dir, f'{material}_loss_first_{n_plot}_samples.png')
    plt.savefig(pdf_line_path)
    plt.savefig(png_line_path, dpi=300, bbox_inches='tight')
    plt.close()
