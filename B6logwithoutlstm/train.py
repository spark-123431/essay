...
def train_model(data_dir, material, base_model_path, device, epochs, verbose=False):
    ...
    # === 附加图：从第 50 轮开始绘制 ===
    if len(epochs_list) >= 30:
        # 筛选 epoch >= 50 的数据点
        filtered_epochs = [e for e in epochs_list if e >= 30]
        filtered_train = train_losses[29:]  # 索引从 epoch 50 起，即 index=49
        filtered_valid = valid_losses[29:]

        fig, ax = plt.subplots()
        ax.plot(filtered_epochs, filtered_train, label='Train Loss (Epoch ≥ 30)')
        ax.plot(filtered_epochs, filtered_valid, label='Val Loss (Epoch ≥ 30)')
        ...
        ax.set_xlabel('Epoch')
        ax.set_ylabel('Loss')
        ax.set_title(f'Training Progress (From Epoch 30): {material}')
        ax.legend()
        fig.tight_layout()
        zoom_fig_path = os.path.join(progress_dir, f"{material}_from_epoch30.pdf")
